...
export default function Register({ navigation }) {
  ...
  function createUser(user) {
    ...
    axios
      .post("https://find-spot.herokuapp.com/register", user) //Envia por post la a crear
      .then((res) => {
  ...
  function validate(input) {
    ...
    } else if (!/^[^@]+@[^@]+\.[a-zA-Z]{2,}$/i.test(input.email)) {
    ...
    if (!input.password) {
      ...
      error.password = "Contraseña insegura";
    }
    if (input.password !== input.passwordRep) {
      error.passwordRep = "Contraseña no coincide";
    }
    if (!validateAge()) {
      error.dateOfBirth = "Debes ser mayor de edad";
    }
  ...
  function validateAge() {
    //Valida que la edad sea 18 o mas
  ...
  const onChange = (event, selectedDate) => {
    ...
    if (currentDate) {
      setInput((prev) => ({
        ...prev,
        dateOfBirth: currentDate.toISOString().slice(0, -14),
      }));
    }
  };
